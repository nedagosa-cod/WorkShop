...
const ButtonState = ({ pos, name }) => {
  const { getScene } = useContext(MapContext);
  return (
    <div
      className={"btn-state " + pos}
      onClick={() => {
        getScene(pos);
      }}
    >
      ...
      <div className="btn-state__subbtn">{name}</div>
      <div className="btn-state__shadow"></div>
    </div>
  );
};
